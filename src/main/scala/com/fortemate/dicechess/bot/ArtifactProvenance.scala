...
private[bot] object ArtifactProvenance:
  ...
  def inspect(pathValue: String, expectedValue: Option[String], configuredId: Option[String]): ArtifactProvenance =
    ...
    expectedValue.foreach { raw =>
      val expected = raw.stripPrefix("sha256:")
      if !Sha256.matches(expected) then
      ...
      if !actual.equalsIgnoreCase(expected) then
        sys.error(s"SHA-256 mismatch for '$pathValue' (expected ${expected.toLowerCase}, got $actual)")
    }
